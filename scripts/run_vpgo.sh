...
STAGE1_TRI_MAX_PROJECT_ERROR_VALUES=(12.0)
STAGE1_FILTER_MAX_REPROJ_ERROR_VALUES=(12.0)
...
printf "run_idx\tstatus\telapsed_sec\toutput_dir\tlog_file\tstage1_max_iterations\tstage1_tri_max_project_error\tstage1_filter_max_reproj_error\tstage2_max_iterations\tstage2_tri_max_project_error\tstage2_filter_max_reproj_error\tstage3_max_iterations\tstage3_tri_max_project_error\tstage3_filter_max_reproj_error\n" > "$summary_file"
            ...
            for s3_iter in "${STAGE3_MAX_ITER_VALUES[@]}"; do
              for s3_tri in "${STAGE3_TRI_MAX_PROJECT_ERROR_VALUES[@]}"; do
                for s3_filter in "${STAGE3_FILTER_MAX_REPROJ_ERROR_VALUES[@]}"; do
                  ...
                  tag="s1i$(sanitize_value "$s1_iter")_s1tri$(sanitize_value "$s1_tri")_s1f$(sanitize_value "$s1_filter")_s2i$(sanitize_value "$s2_iter")_s2tri$(sanitize_value "$s2_tri")_s2f$(sanitize_value "$s2_filter")_s3i$(sanitize_value "$s3_iter")_s3tri$(sanitize_value "$s3_tri")_s3f$(sanitize_value "$s3_filter")"
                  ...
                  printf "%d\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
